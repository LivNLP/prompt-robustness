...
def run_model(args):
    ...
    if args.label_map is not None:
        ...
        if args.label_token_file != None:
            #print(args.label_map)
            with open(args.label_token_file) as f:
    ...
    for i in range(args.iters):
        ...
        if dev_metric > best_dev_metric:
            ...
            result = {"best_dev_metric": best_dev_metric, 
    ...
    dp = str(args.result_filename).split("/")[-1].split("datapoint-")[-1].replace(".json", "")
    with open("{}/args_dp-{}.json".format(savedir, dp), mode="w") as f:
        args.train = str(args.train) 
        args.dev = str(args.dev) 
